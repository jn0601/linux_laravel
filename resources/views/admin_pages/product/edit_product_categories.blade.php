									...
									<div class="x_content">
										...
										<div class="field item form-group">
											...
											<div class="col-md-10 col-sm-10">
												...
												<br>
												<img src="{{URL::to('public/backend/uploads/product_categories/'.$edit_value->image)}}"
												...
												<select name="category_id" class="form-control">
													...
													@if($edit_value->parent_id == 0)
														...
														@foreach($categories as $key => $cate)
															@if($cate->level != 3)
																@if($cate->level == 1)
																<option style="color: red;" value="{{$cate->id}}">Cấp 1. {{$cate->name}}</option>
																@else
																<option style="color: green;" value="{{$cate->id}}">Cấp 2. {{$cate->name}}</option>
																@endif
															@endif
														@endforeach
													@else
														@foreach($categories as $key => $cate)
															...
															@if($cate->id==$edit_value->parent_id)
																@if($cate->level == 1)
																	<option style="color: red;" selected value="{{$cate->id}}">Cấp 1. {{$cate->name}}</option>
																@else
																	<option style="color: green;" selected value="{{$cate->id}}">Cấp 2. {{$cate->name}}</option>
																@endif
															<!-- nếu chưa thuộc danh mục nào thì hiện danh sách -->
															@else
																@if($cate->id != $edit_value->id)
																	@if($cate->level != 3)
																		@if($cate->level == 1)
																			<option style="color: red;" value="{{$cate->id}}">Cấp 1. {{$cate->name}}</option>
																		@else
